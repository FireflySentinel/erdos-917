...
lemma rectangle_bound (hq : 4 ≤ q) {A B : Finset (ZMod q)}
    (ha : A.Nonempty) (hb : B.Nonempty)
    (hab : ∀ a ∈ A, ∀ b ∈ B, b ∈ successors a) : A.card + B.card ≤ 4 := by
  obtain ⟨a, ha⟩ := ha
  obtain ⟨b, hb⟩ := hb
  have hA : A ⊆ predecessors b := fun x hx => mem_predecessors_iff.mpr (hab x hx b hb)
  have hB : B ⊆ successors a := fun y hy => hab a ha y hy
  have hAc : A.card ≤ 3 := by simpa [predecessors_card hq] using card_le_card hA
  have hBc : B.card ≤ 3 := by simpa [successors_card hq] using card_le_card hB
  have hA1 (h : 3 ≤ B.card) : A.card ≤ 1 := by
    apply card_le_one.mpr
    intro x hx y hy
    apply successors_injective hq
    have hx' : B = successors x := eq_of_subset_of_card_le (fun z hz => hab x hx z hz)
      (by rw [successors_card hq]; exact h)
    have hy' : B = successors y := eq_of_subset_of_card_le (fun z hz => hab y hy z hz)
      (by rw [successors_card hq]; exact h)
    exact hx'.symm.trans hy'
  have hB1 (h : 3 ≤ A.card) : B.card ≤ 1 := by
    apply card_le_one.mpr
    intro x hx y hy
    apply predecessors_injective hq
    have hx' : A = predecessors x := eq_of_subset_of_card_le
      (fun z hz => mem_predecessors_iff.mpr (hab z hz x hx))
      (by rw [predecessors_card hq]; exact h)
    have hy' : A = predecessors y := eq_of_subset_of_card_le
      (fun z hz => mem_predecessors_iff.mpr (hab z hz y hy))
      (by rw [predecessors_card hq]; exact h)
    exact hx'.symm.trans hy'
  omega

lemma third_level {x y z : CoreVertex q}
...
lemma clique_levels {C : Finset (CoreVertex q)} (hc : (C : Set (CoreVertex q)).Pairwise coreAdj)
    {x y : CoreVertex q} (hx : x ∈ C) (hy : y ∈ C) (hij : x.level < y.level) :
...
lemma place_injective_of_opposite {C : Finset (CoreVertex q)} (hc : (C : Set (CoreVertex q)).Pairwise coreAdj)
    {i : Fin (q + 1)} {z : CoreVertex q} (hz : z ∈ C) (hiz : i ≠ z.level) :
    Set.InjOn CoreVertex.place (↑(C.filter (fun x => x.level = i)) : Set (CoreVertex q)) := by
...
lemma coreClique_card_le (hq : 4 ≤ q) {C : Finset (CoreVertex q)}
    (hc : (C : Set (CoreVertex q)).Pairwise coreAdj) : C.card ≤ 4 := by
  classical
  by_cases hall : ∀ x ∈ C, ∀ y ∈ C, x.level = y.level
  · have hinj : Set.InjOn CoreVertex.copy (C : Set (CoreVertex q)) := by
      intro x hx y hy he
      by_contra hn
      exact ((coreAdj_same (hall x hx y hy)).mp (hc hx hy hn)).1 he
    calc
      C.card = (C.image CoreVertex.copy).card := (card_image_of_injOn hinj).symm
      _ ≤ (univ : Finset (Fin 4)).card := card_le_card (subset_univ _)
      _ = 4 := by simp
  push Not at hall
  obtain ⟨x, hx, y, hy, hxy⟩ := hall
  suffices h : ∀ x ∈ C, ∀ y ∈ C, x.level < y.level → C.card ≤ 4 by
  ...
  have hrect := rectangle_bound hq hAb hBb (by
    intro a ha b hb
...
lemma lineClique_card_le (P : Plane q) (l : P.Line) {C : Finset (CoreVertex q)}
    (hc : (C : Set (CoreVertex q)).Pairwise coreAdj)
    (hp : ∀ x ∈ C, P.point l x.level = x.place) : C.card ≤ 3 := by
  classical
  by_cases hall : ∀ x ∈ C, ∀ y ∈ C, x.level = y.level
  · have hinj : Set.InjOn CoreVertex.kind (C : Set (CoreVertex q)) := by
      intro x hx y hy he
      by_contra hn
    ...
    calc
      ...
      _ ≤ (univ : Finset (ZMod 3)).card := card_le_card (subset_univ _)
      _ = 3 := by simp
  push Not at hall
  obtain ⟨x, hx, y, hy, hxy⟩ := hall
  suffices h : ∀ x ∈ C, ∀ y ∈ C, x.level < y.level → C.card ≤ 3 by
    rcases lt_or_gt_of_ne hxy with hxy | hxy
    · exact h x hx y hy hxy
    · exact h y hy x hx hxy
  intro x hx y hy hxy
  have hlevels := clique_levels hc hx hy hxy
  have hcard (i : Fin (q + 1)) (z : CoreVertex q) (hz : z ∈ C) (hiz : i ≠ z.level) :
...
/-- The complete AEHK graph contains no five-clique. -/
theorem planeGraph_cliqueFree (P : Plane q) (hq : 4 ≤ q) : (planeGraph P).CliqueFree 5 := by
  classical
  intro C hC
  have hc := hC.isClique
  have hcore : (C.toLeft : Set (CoreVertex q)).Pairwise coreAdj := by
    intro x hx y hy hxy
