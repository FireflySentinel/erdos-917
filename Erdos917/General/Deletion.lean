import Erdos917.General.Assembly
...
lemma common_color {i j : (Part n)} (hij : i ≠ j) {α β : (Palette n)}
...
lemma assemble_except (c : (Part n) → ModuleVertex X Y → (Palette n)) (u v : AssemblyVertex n X Y)
...
/-- Every cross-module edge can be the sole monochromatic edge in an `(2 n + 7)`-color assignment. -/
theorem delete_cross_coloring [DecidableEq X] [DecidableEq Y]
    (hS : CriticalData S (2*n+6)) (hT : CriticalData T (2*n+5)) {D : ℕ} (hz : Scaffold Z D)
    (i j : (Part n)) (hij : i ≠ j) (p q : X × Y) (hpq : ¬ Z.Adj (i,p) (j,q)) :
    ∃ f : AssemblyVertex n X Y → (Palette n),
      ProperExcept (assemblyGraph S T Z) f (i,a p) (j,a q) := by
  classical
  obtain ⟨r,hri,hrj,hr⟩ := hz.fill i j hij p q hpq
  have hex (k : (Part n)) := Module.singleton_active_coloring S T (by simp; omega) hS hT (r k)
...
/-- Every internal edge admits an `(2 n + 7)`-color deletion assignment of the whole graph. -/
theorem delete_internal_coloring [DecidableEq X] [DecidableEq Y] [Nonempty X]
    (hS : CriticalData S (2*n+6)) (hT : CriticalData T (2*n+5)) {D : ℕ} (hz : Scaffold Z D)
    (i : (Part n)) (u v : ModuleVertex X Y) (huv : (moduleGraph S T).Adj u v) :
    ∃ f : AssemblyVertex n X Y → (Palette n),
      ProperExcept (assemblyGraph S T Z) f (i,u) (i,v) := by
  classical
  obtain ⟨r,hr⟩ := hz.all_but_one i
  obtain ⟨d,hd,hda⟩ := Module.delete_edge_coloring S T (by simp; omega) hS hT u v huv
    (some (i,false)) (some (i,true)) (by simp)
  have hex (k : (Part n)) := Module.singleton_active_coloring S T (by simp; omega) hS hT (r k)
    (some (k,false)) (some (k,true)) none (by simp) (by simp) (by simp)
  choose c hc hcγ hcs hct using hex
  let f : (Part n) → ModuleVertex X Y → (Palette n) := fun k => if k = i then d else c k
...
theorem delete_edge_colorable [DecidableEq X] [DecidableEq Y] [Nonempty X]
    (hS : CriticalData S (2*n+6)) (hT : CriticalData T (2*n+5)) {D : ℕ} (hz : Scaffold Z D)
    (u v : AssemblyVertex n X Y) (huv : (assemblyGraph S T Z).Adj u v) :
    ((assemblyGraph S T Z).deleteEdges {s(u,v)}).Colorable (2*n+7) := by
  have hex : ∃ f : AssemblyVertex n X Y → (Palette n),ProperExcept (assemblyGraph S T Z) f u v := by
  ...
  convert c.colorable using 1; simp [Palette]; omega
