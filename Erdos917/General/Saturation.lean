...
/-- A `K_(n+3)`-free graph whose missing edges have common `(n+1)`-cliques. -/
structure Saturated (n : ℕ) {V : Type*} (H : SimpleGraph V) : Prop where
  free : H.CliqueFree (n+3)
...
lemma no_clique (hH : H.CliqueFree (n+3)) (r : (Part n) → X × Y) :
    ¬ (∀ i j,i ≠ j → (blowupGraph H e).Adj (i,r i) (j,r j)) := by
  intro hr
  let c : Copy (completeGraph (Part n)) H := {
...
omit [Fintype V] in
lemma common_including_equal (hH : Saturated n H) (hn : ∀ z,∃ w,z ≠ w ∧ ¬ H.Adj z w)
    (z w : V) (hzw : ¬ H.Adj z w) : ∃ t : Fin (n+1) → V,
    (∀ i j,i ≠ j → H.Adj (t i) (t j)) ∧ ∀ i,H.Adj z (t i) ∧ H.Adj w (t i) := by
  by_cases he : z = w
  · subst w
    obtain ⟨w,hne,hnw⟩ := hn z
    obtain ⟨t,ht,hzt⟩ := hH.common z w hne hnw
    exact ⟨t,ht,fun i => ⟨(hzt i).1,(hzt i).1⟩⟩
  · exact hH.common z w he hzw

omit [Fintype X] [Fintype Y] [Fintype V] in
lemma fill (hH : Saturated n H) (hn : ∀ z,∃ w,z ≠ w ∧ ¬ H.Adj z w)
    (i j : (Part n)) (hij : i ≠ j) (p q : X × Y)
    (hpq : ¬ (blowupGraph H e).Adj (i,p) (j,q)) :
    ∃ r : (Part n) → X × Y,r i = p ∧ r j = q ∧
      ∀ k l,k ≠ l → ¬ ((k = i ∧ l = j) ∨ (k = j ∧ l = i)) →
        (blowupGraph H e).Adj (k,r k) (l,r l) := by
  classical
  obtain ⟨t,ht,hpt⟩ := common_including_equal H hH hn (label e p) (label e q)
    (fun h => hpq ⟨hij,h⟩)
  let R : Finset (Part n) := univ \ {i,j}
  have hR : R.card = n+1 := by simp [R,card_sdiff_of_subset (subset_univ _),hij,Part]
  let idx : R ≃ Fin (n+1) := R.equivFinOfCardEq hR
  let lift : V → X × Y := fun z => (Classical.arbitrary X,e.symm (z,false))
  have hlift (z : V) : label e (lift z) = z := by simp [lift,label]
  let r : (Part n) → X × Y := fun k => if hk : k = i then p else if hj : k = j then q
    else lift (t (idx ⟨k,by simp [R,hk,hj]⟩))
  have hri : r i = p := by simp [r]
  have hrj : r j = q := by simp [r,hij.symm]
  have hrk (k : (Part n)) (hk : k ≠ i) (hj : k ≠ j) :
...
lemma degree_bound [DecidableRel H.Adj] {d : ℕ} (hd : ∀ z,H.degree z ≤ d)
    (i j : (Part n)) (_hij : i ≠ j) (q : X × Y) :
    Nat.card {p : X × Y // (blowupGraph H e).Adj (i,p) (j,q)} ≤ (2*n+4) * Fintype.card X * d := by
  classical
  ...
  calc
    ...
    _ = 2 * Fintype.card X * d := by ring
    _ ≤ (2*n+4) * Fintype.card X * d :=
      Nat.mul_le_mul_right d (Nat.mul_le_mul_right _ (by omega))

/-- All scaffold properties follow from the supplied saturated graph, including equal-label edges. -/
theorem scaffold [DecidableRel H.Adj] [Nonempty V] (hH : Saturated n H)
    {d : ℕ} (hd : ∀ z,H.degree z ≤ d) (hsmall : d < Fintype.card V-1) :
    Scaffold (blowupGraph (n := n) (X := X) H e) ((2*n+4) * Fintype.card X * d) := by
  classical
  have hn := nonneighbor H hd hsmall
  refine ⟨degree_bound H e hd,no_clique H e hH.free,fill H e hH hn,?_⟩
  intro i
  obtain ⟨j,hj⟩ : ∃ j : (Part n),j ≠ i := exists_ne i
