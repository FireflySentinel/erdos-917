...
For \(k=12\), the density is \(2/5>3/8\).
...
Toft's coefficient \(b_k\), and \eqref{eq:conjecture} fails.
In particular, \eqref{eq:conjecture} fails for every multiple
of three at least twelve. For \(k=12\),
\[
...
Formula~\eqref{eq:conjecture} would force the limit, and hence
the limsup, to equal \(\tfrac12(1-1/\lfloor k/3\rfloor)\),
contradicting the lower bound. Taking \(k=12\) gives the displayed
inequality.
At \(k=9\), all three coefficients equal \(1/3\).
\end{proof}

The failure of \eqref{eq:conjecture} for \(3\nmid k\) was already
implied by Toft's constructions; the new cases are \(3\mid k\),
\(k\ge12\).
...
At \(t=2\), the graph \(H\) is edgeless and the two active sets
are completely joined, as in Pegden's two-module construction
\cite[Section~2.1]{Pegden}. The limiting density is \(1/4\),
giving no improvement for \(k=6\) or for \(k=7\) after taking a cone.
...
For fixed \(k\ge4\) and sufficiently large \(n\), Luo, Ma, and Yang
\cite[Theorem~1.1]{LMY} give
\[
 f_k(n)\le e(T_{k-2}(n))-\frac{n^2}{36(k-1)^2}
 \le\left(\frac12-\frac1{2(k-2)}-\frac1{36(k-1)^2}\right)n^2,
\]
where \(T_{k-2}(n)\) is the balanced complete \((k-2)\)-partite graph.
For even \(k\), the difference between this upper coefficient and
\(c_k\) is \(1/(2(k-2))-1/(36(k-1)^2)\).
At \(k=12\), the upper coefficient is \(9/20-1/4356\).
...
Hanson and Seyffarth \cite{HansonSeyffarth} constructed maximal
triangle-free graphs of maximum degree \(O(\sqrt n)\), and
F\"uredi and Seress \cite{FurediSeress} improved the constant.
We use \cite[Section~4, Example~2]{AEHK}, with \(k=t\) and one
added vertex per line, under the sufficient condition \(Q\ge2t\).
The case \(t=3\) is \cite[Example~2.2]{FurediSeress}; its core has
no edges between different levels. We give the saturation verification
and exact degree counts below.

\begin{lemma}[after {\cite[Example~2]{AEHK}}]
...
For integers \(s\) sufficiently large that \(Q=9^s\ge2t\), set
\[
...
Corollary~\ref{cor:general} is formalized as
\nolinkurl{Erdos917.corollary_two}, using \texttt{fk},
\texttt{erdosCoefficient}, and \texttt{toftCoefficient} for the
extremal function and the two comparison coefficients; its \(k=12\)
instance is \nolinkurl{Erdos917.f12_limsup_ge_two_fifths}.
...
\bibitem{FurediSeress}
Z.~F\"uredi and \'A.~Seress,
Maximal triangle-free graphs with restrictions on the degrees,
\emph{J. Graph Theory} \textbf{18} (1994), 11--24.
\href{https://doi.org/10.1002/jgt.3190180103}{doi:10.1002/jgt.3190180103}.

\bibitem{HansonSeyffarth}
D.~Hanson and K.~Seyffarth,
\(k\)-saturated graphs of prescribed maximum degree,
\emph{Congr. Numer.} \textbf{42} (1984), 169--182.
