...
The Lean development covers \(k=12\); the preceding Remark on other
chromatic numbers is not included.
Lemma~\ref{lem:module}, Proposition~\ref{prop:conversion} (criticality,
order, and the exact edge count \eqref{eq:exact-edges}), and
Lemma~\ref{lem:saturation} have been formalized in Lean~4 with Mathlib.
The finite fields are constructed using Mathlib's \texttt{GaloisField}.
The formalization proves the limit \(e(G_s)/|V(G_s)|^2\to2/5\),
defines \(f_{12}\) under the edge-deletion convention, and proves
that \(f_{12}(n)/n^2\) has no limit below \(2/5\), with no external
mathematical hypothesis. The corresponding declarations are
\nolinkurl{Erdos917.f12_not_density_below_two_fifths} and
\nolinkurl{Erdos917.not_density_three_eighths}, the latter stating the
failure of the proposed limit \(3/8\).
