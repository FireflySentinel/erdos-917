...
\(n\) vertices. Erd\H{o}s \cite{Erdos1969} conjectured that, for fixed \(k\ge6\),
...
and Jensen \cite{Jensen2002}. We show that the coefficient in
\eqref{eq:conjecture} is not optimal for \(k=12\). The problem is
recorded as Erd\H{o}s Problem 917 \cite{Erdos917}.
...
The following properties are the \(U(11,10)\) case of Pegden's
module lemma \cite[Lemma 2.5]{Pegden}, except that Pegden assumes the
factor graphs to be triangle-free. The proof below shows directly that
this hypothesis is not needed for the properties used here.
...
\]
Both cycles are odd and have length at least three. Complete graphs
and odd cycles have every proper subgraph colorable with one fewer
color, and this property is preserved under joins. Indeed, deleting a
vertex or an edge within one factor saves a color in that factor; if a
joining edge \(xy\) is deleted, color each factor so that a prescribed
color occurs only at \(x\), respectively \(y\), and identify those two
colors. Hence \(S\) is eleven-chromatic with every proper subgraph
ten-colorable, while \(T\) is ten-chromatic with every proper subgraph
nine-colorable.
...
colors, some structural vertex has color \(\alpha\). The \(\ell\)
active vertices in its row avoid \(\alpha\), so the pigeonhole
